...
cpu 的单核性能提升接近瓶颈，多核是未来 cpu 的趋势。而多线程就能充分利用多核技术。但使用多线程不一定只是为了追求性能，使用多线程的意义也在于提升处理任务数量（请求个数）的能力，比如说即时通讯系统中，发起多个会话就是如此。不开线程就没办法，发起新的一个会话。当然多进程也能做这些事情。
...
#### 附录：std::async 策略与 future 规则

- 策略
  - std::launch::async：一定在新线程中异步启动任务。
  - std::launch::deferred：不启动线程，任务在调用 future.wait()/get() 的线程中惰性执行。
  - 默认策略（不指定时实现可选 async 或 deferred）：实现可自由选择，行为因平台而异。
- future 无 detach
  - std::future/std::shared_future 均无“detach”概念；共享状态的生命周期由最后一个 future 对象管理。
- 析构是否阻塞（关键规则）
  - 若使用了 std::launch::async 启动且该共享状态的“最后一个” future/shared_future 在未 wait()/get() 的情况下析构，则该析构将阻塞直至异步任务结束（防止“后台悬挂线程”）。
  - 若为 deferred，或已 wait()/get() 完成，析构不再额外阻塞。
  - 若策略为默认且实现选择了 async，则按上条处理；若选择 deferred，则不阻塞。
- 其他提示
  - shared_future 允许多消费者；同样遵循“最后一个引用者析构阻塞”的规则（当且仅当该共享状态是以 async 启动且仍在运行）。
  - 判断是否持有共享状态：future.valid()。

#### 附录：线程与并行库的异常传播与处理

- std::thread
  - 线程函数内未捕获异常将调用 std::terminate；务必在线程函数内 try/catch 并通过回调或 std::promise 上报。
- std::async/std::packaged_task
  - 异常被捕获并存入共享状态，在 future.get() 处重新抛出；未 get 的情况下异常不会“丢失”，但若为 async 且最后一个 future 析构，会先等待任务结束。
  - std::promise::set_exception 可手动传播异常；std::packaged_task 会自动把任务异常放入共享状态。
- 线程池/并行库
  - TBB/OpenMP/并行算法通常在调度边界聚合或转发异常；需在提交/等待位置包裹 try/catch，并查看库文档的传播策略（如 OpenMP 的结构化块、TBB 的聚合 rethrow 等）。
- 实践建议
  - 明确异常边界：线程函数内捕获并转换为状态码/回调/Promise；上层统一在 future.get()/wait() 或任务提交点做收敛与处理。
