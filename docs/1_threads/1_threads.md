...
cpu 的单核性能提升接近瓶颈，多核是未来 cpu 的趋势。用了多线程，才能充分利用多核技术带来的性能提升。但使用多线程不一定只是为了追求性能，使用多线程的意义也在于提升处理任务数量（请求个数）的能力，比如说即时通讯系统中，发起多个会话就是如此。不开线程就没办法发起新的一个会话。可以说多线程是比较重要，且属于程序开发中比较基础的能力。而多线程的使用可以从两个角度出发去理解：
...
随着线程池和并行库等高级工具的广泛应用，线程的管理逻辑被封装在内核中，外部调用者无需感知线程的具体创建、调度和销毁过程。这种抽象设计带来了便利，但也容易导致概念混淆。例如，在使用并行库执行任务时，开发者可能默认是通过多线程实现并行，但实际上底层实现可能采用协程、异步 I/O 或其他非线程技术。以`OpenMP`的嵌套并行为例，其内层并行任务在默认配置下并不会创建新的线程，而是复用外层线程资源，因此很多并行库的接口都是和 task 相关，因为并行实现也许不是依靠线程，一个线程实际上可能是运行了多个 task。
...
| 操作类型                                    | CPU 时钟周期范围 | 时间范围                      |
| ------------------------------------------- | ---------------- | ----------------------------- |
| L1 缓存读取                                 | 3–5 周期         | ~1–2 ns                       |
| L2 缓存读取                                 | 12–20 周期       | ~4–7 ns                       |
| L3 缓存读取                                 | 30–60 周期       | ~10–20 ns                     |
| 主内存读取                                  | 100–300 周期     | ~33–100 ns                    |
| NUMA（不同插槽的原子操作/CAS，估计值）      | 600–2k 周期      | ~200–667 ns                   |
| NUMA（不同插槽的 L3 缓存读取）              | 100–400 周期     | ~33–133 ns                    |
| 分配+释放配对（小对象，内存池方式）         | 10–100 周期      | ~3–33 ns                      |
| 分配+释放配对（小对象，new 和 delete 方式） | 1k–5k 周期       | ~333–1,667 ns                 |
| -------------------------                   | --------         | ----------                    |
| 内核调用                                    | 1k–5k 周期       | ~333–1,667 ns                 |
| 线程上下文切换（直接成本）                  | 1k–10k 周期      | ~333–3,333 ns (≈0.33–3.33 µs) |
| 线程上下文切换（总成本，包括缓存失效）      | 10k–100k 周期    | ~3.3–33.3 µs                  |
| -------------------------                   | --------         | ----------                    |
| 磁盘访问（SSD）                             | 150k–450k 周期   | ~50–150 µs                    |
| 磁盘访问（HDD）                             | 15m–30m 周期     | ~5–10 ms                      |
| 线程创建                                    | 30k–300k 周期    | ~10–100 µs                    |
| 线程销毁                                    | 30k–300k 周期    | ~10–100 µs                    |
p.s.: 数据来源自AI，使用同一问题在不同AI多次提问，虽说结果略有差异，但量级关系一致，具有一定参考价值吧。

在多线程性能分析中，虽然具体操作开销因硬件架构、系统负载等因素存在差异，但不同操作的开销量级关系具有普遍参考价值。深入理解这些量级差异及其影响因素，是实现系统性能优化的核心。

对于多线程来说，当单个任务执行时间达到毫秒级别时，正常使用的时候不太需要考虑创建线程的开销；但对于本身是微秒级别的任务来说，在特定场景下创建线程可能引发性能问题。
...
如果把上述看成一个状态机，`New`和`Terminated`其实就是起点和终点，相对重要的是其余三个状态。

- 什么时候会进入`waiting/Blocked`状态？
  - 拿不到锁。`mtx.lock();`如果拿不到锁，当前锁就进入`waiting/Blocked`了。（操作系统的线程调度器会感知到锁的状态变化。它会从等待该锁的线程队列中选择一个线程（通常按照一定的调度策略，如先进先出），并将其唤醒。也就是说有一个类似`notify`的行为。）
  - 调用`wait()`的时候，是先拿锁，再检查条件的。
  - 调用`sleep()`
  - 发起内存/硬盘/网络等`io`操作
- **只有`Running`状态的才在真实消耗 CPU 计算资源**
- `Ready`状态分配到时间片就会切换到`Running`，时间片消耗完或者主动让出（使用`std::yield`）就会回到`Ready`。
- 线程处于`Waiting`状态的时候不会影响线程调度。也就是说当前程序，**有很多个`Waiting`状态的线程，和没有`Waiting`状态的，两者线程调度开销几乎一致。**

### 1. 多线程的创建和管理手段

c++多线程创建和管理的手段就三种：`std::thread`、`std::async`和`std::packaged_task`。前两个偏向于创建，后一个偏向于管理。

`std::thread`是操作系统级别的线程封装，不关注返回值，不关注线程管理； `std::async`和`std::packaged_task`是任务级别的线程封装，关注返回值，关注线程管理。前者偏向于自动管理，是一个 task，后者偏向于手动管理。
...
当有多个线程访问相同的共享数据时，做出的操作往往是不安全的。这就需要线程同步。所谓的线程同步，就是指多线程通过特定的设置（如互斥量、事件对象、临界区）来控制线程之间的执行顺序。同步的目的是协同、协助、互相配合线程的运行，而不是同时进行。线程同步通过建立线程之间的执行顺序关系，确保线程按预定的顺序运行。例如，一个线程完成任务后，另一个线程才开始执行。如果没有同步机制，线程将各自独立运行，可能导致资源竞争和数据不一致的问题。
...
`std::thread` 不能直接传递引用类型参数。`std::thread` 在创建线程时会复制传入的参数。

这是一个强调安全性的设计决策，强制开发人员显式地表明他们希望传递引用，而不是值。不是说不能不用`std::ref`就实现不了传引用，而是说这样做会更不安全，更容易出错。

因为主线程和子线程是两个声明周期，如果直接传递引用，当主线程中的对象销毁后，线程函数中的引用就会成为悬空引用，访问悬空引用会导致未定义行为。

为了保证安全性，如果要使用`T&`则需要开发人员主动使用`std::ref`，否则会报错。因此来避免不小心而导致的悬空引用。
...
`std::future`通常由`std::async`、`std::packaged_task`等创建。若创建了`std::future`对象，但未调用`get()`或`wait()`，其析构行为与来源有关：

- 来自 `std::async` 且采用 `std::launch::async` 的任务：当最后一个引用该共享状态的 `std::future` 被销毁时，标准允许析构阻塞直至任务完成（实现可选择阻塞）。因此，可能出现析构点短暂阻塞的情况。
- 来自 `std::async` 且采用 `std::launch::deferred` 的任务：任务仅在 `get()/wait()` 时执行；若未等待即销毁，不会阻塞，但任务也不会执行。
- 来自 `std::promise`/`std::packaged_task`：销毁 `std::future` 不会阻塞被执行的任务（任务生命周期独立管理）。

注意：C++ 标准库中没有 `std::future::detach()`。如需“真正后台执行且无需等待结果”，可采用以下方式：

- 使用 `std::async(std::launch::async, ...)` 并在合适位置调用 `wait()`（即使忽略结果也应显式等待）；
- 或改用 `std::thread` 并 `detach()`；
- C++20 起可用 `std::jthread` 搭配停止令牌管理任务生命周期。
...
- `std::thread`：线程函数内未捕获异常会调用 `std::terminate`。需在线程函数内捕获并通过 `promise/future` 或回调上报。
- `std::async`：异常被捕获并存入共享状态，调用 `future.get()` 时重新抛出。
- 第三方并行库：如 TBB/OpenMP 通常将子任务异常聚合/传播到调用点，需查阅库文档并在并行边界捕获。
